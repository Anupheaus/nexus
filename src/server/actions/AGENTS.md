...
| `restAuthMiddleware.ts` | `runRestAuth` — the auth-gate step used by `registerRestActions.ts`; runs `auth.onResolveRestConnection` (if supplied) before the isPublic gate and `validateRestSession` |
